...
to the repository and whether installed dependencies match `package-lock.json`.
If updates are available or dependencies are out of date, it will show a message
to the user that there is an update available and will tell them to hit the `u`
key to update the workshop. The update automation only runs `npm install` when
the lockfile and installed packages are out of sync. If you want to run anything
after the update, add a `postupdate` script to `epicshop.scripts` in the root
`package.json`:
