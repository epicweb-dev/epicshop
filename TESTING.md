...
This guide covers the testing setup and best practices for the Epic Workshop
project.
...
test('should handle async operations', async () => {
	...
	try {
		// Your test code here
		await expectToThrow(() => asyncFunction(), 'Expected error message')
	} finally {
		env.cleanup()
	}
})
...
1. **Follow naming conventions**: `*.test.ts` for unit tests, `*.spec.ts` for
   E2E tests
