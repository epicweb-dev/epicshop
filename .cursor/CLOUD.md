...
Epic Workshop (epicshop) is a React Router v7 monorepo providing a local learning environment for EpicWeb.dev workshops.
...
- **Nx orchestration**: All build/test/lint commands use nx for automatic dependency ordering and caching
- **Server-only modules**: Files with `.server` suffix in `@epic-web/workshop-utils` are server-only and cannot be imported in client components
- **Always build before testing**: Run `npm run build` before `npm run dev` when testing client-side changes
- **Port auto-assignment**: The app starts on port 5639 by default but will auto-find an available port if occupied
...
- `@epic-web/workshop-presence` (packages/workshop-presence): Optional real-time presence
