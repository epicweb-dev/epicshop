...
// Maximum number of log lines to keep per sidecar process
const MAX_SIDECAR_LOG_LINES = 500
...
export function startSidecarProcess(name: string, command: string) {
	...
	function addOutputLine(type: 'stdout' | 'stderr', content: string) {
		...
		// Keep only the last MAX_SIDECAR_LOG_LINES entries
		if (output.length > MAX_SIDECAR_LOG_LINES) {
			output.shift()
		}
	}
